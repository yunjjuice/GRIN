...
class Painter:
    ...
    def __init__(self, gpu=0):
        rootPath = os.path.dirname(os.path.abspath(os.path.dirname(os.path.abspath(os.path.dirname(__file__)))))
        print("start")
        ...
        print(rootPath)
        if not os.path.isfile(str(rootPath)+"/downloads/painting_model/unet_128_standard"):
            print("./models/unet_128_standard not found. Please download them from http://paintschainer.preferred.tech/downloads/")
        if not os.path.isfile(str(rootPath)+"/downloads/painting_model/unet_512_standard"):
        ...
        if self.gpu >= 0:
            self.cnn_128.to_device(self.gpu)
            self.cnn_512.to_device(self.gpu)
        #lnn = lnet.LNET()
        ...
        serializers.load_npz(
            str(rootPath)+"/downloads/painting_model/unet_128_standard", self.cnn_128)
        ...
        serializers.load_npz(
            str(rootPath)+"/downloads/painting_model/unet_512_standard", self.cnn_512)
    ...
    def colorize(self, id_str, step='C', blur=0, s_size=128,colorize_format="jpg"):
        ...
        dataset = ImageAndRefDataset(
            [id_str + ".png"], self.root, self.root)

        _ = {'S': True, 'L': False, 'C': True}
        sample = dataset.get_example(0, minimize=False, blur=blur, s_size=s_size)
